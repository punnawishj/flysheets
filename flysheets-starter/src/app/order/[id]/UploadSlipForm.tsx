...
// Keeps only a plain ASCII extension like ".jpg" -- falls back to no
...
export default function UploadSlipForm({ orderId, userId }: { orderId: string; userId: string }) {
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    //
    // Supabase Storage rejects object keys with non-ASCII characters,
    // spaces, or certain punctuation ("Invalid key"), and a phone photo's
    // filename can contain any of those. Only the extension from the
    // original filename is kept; the rest of the key is just ids.
    const slipPath = `${userId}/${orderId}-${Date.now()}${safeExtension(file.name)}`;
