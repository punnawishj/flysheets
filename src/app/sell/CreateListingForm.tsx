...
export default function CreateListingForm({ userId, subjects }: { userId: string; subjects: string[] }) {
  // Categories are managed by the admin (see /admin/categories) and
  // fetched from the database by the server component that renders this
  // form. If the admin has somehow deleted every category, fall back to
  // a single option so the <select> is never left empty.
  const subjectOptions = subjects.length > 0 ? subjects : ["อื่นๆ"];
